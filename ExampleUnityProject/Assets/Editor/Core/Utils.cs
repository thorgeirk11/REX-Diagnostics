...
namespace Rex.Utilities
{
    public static class Utils
    {
        ...
        public static readonly Regex Assgiment = new Regex(@"^(?<type>\S*\s+)?(?<var>[^ .,=]+)\s*=(?<expr>[^=].*)$", RegexOptions.Compiled | RegexOptions.Singleline);
        ...
        private static MemberDetails DealWithRefParameters(Type t, bool showFullName)
        {
            ...
            if (nested.IsEmpty())
                return new MemberDetails(Syntax._Type(showFullName ? t.FullName : t.Name));
            else
                return new MemberDetails(nested.Concat(new[] { Syntax._Type(t.Name) }));
        ...
        internal static MemberDetails GetCSharpRepresentation(Type t, bool showFullName, List<Type> availableArguments)
        {
            if (t.IsGenericType)
            {
                ...
                details.Add(Syntax._Type(value));

                List<Syntax> genericArguments = GenericArgumentsToSyntax(availableArguments, showFullName);
