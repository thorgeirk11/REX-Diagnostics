...
namespace Rex.Window
{
    ...
    public static class UStrap
    {
        ...
        {
            InputType locks = InputType.None;
        ...
        public static Rect SubRect(this Rect super, int cols = 1, int col = 0, int colSpan = 1, int rows = 1, int row = 0, int rowSpan = 1, float vSpacing = 0.0f, float hSpacing = 0.0f, bool grouped = false)
        {
            ...
            float totalVS = (float)rows * vSpacing;
            float totalHS = (float)cols * hSpacing;
            float colWidth = ((super.width - totalHS) / (float)cols);
            float rowHeight = ((super.height - totalVS) / (float)rows);

            float x = ((hSpacing + colWidth) * (float)col) + hSpacing;
            float y = ((vSpacing + rowHeight) * (float)row) + vSpacing;
            float width = ((colWidth) * (float)colSpan) - hSpacing;
            float height = ((rowHeight) * (float)rowSpan) - vSpacing;
        ...
        {
            float dt = top + padding;
            float dl = left + padding;
            float db = bottom + dt + padding;
            float dr = right + dl + padding;
