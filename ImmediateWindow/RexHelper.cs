...
$@"{codeString}
	...
	{VaribleType(var.Value)} {var.Key} 
	{{ 
		get 
		{{
			if (!Rex.Utilities.RexHelper.Variables.ContainsKey(""{var.Key}""))
				throw new Rex.Utilities.Helpers.AccessingDeletedVariableException() {{ VarName = ""{var.Key}"" }};
			return ({Utils.GetCSharpRepresentation(var.Value.VarType, true).ToString()})Rex.Utilities.RexHelper.Variables[""{var.Key}""].VarValue;
		}} 
		set {{ Rex.Utilities.RexHelper.Variables[""{var.Key}""].VarValue = value; }} 
	}}");
...
{";
	...
	{ 
		...
		private static MemberDetails VaribleType(Varible var) => Utils.GetCSharpRepresentation(var.VarType, true);
		#endregion

		#region Parse
		public static ParseResult ParseAssigment(string parsedCode)
		{
			var match = Utils.Assignment.Match(parsedCode);
			if (match.Success)
			{
