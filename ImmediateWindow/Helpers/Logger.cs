...
namespace Rex.Utilities.Helpers
{
    public static class Logger
    {
        ...
        /// <para>Use: <see cref="Logger"/>.ExtractDetails(new { Data1 = "stuff", MoreData = 42 })</para>
        /// </summary>
        ...
        private static MemberDetails GetValueDetails(object value)
        {
            ...
            if (value == null)
            {
                return new MemberDetails(Syntax.EqualsOp, Syntax.ConstVal("null")) { Value = value };
            }
            else if (ExtractValue(value, out val))
            {
                return new MemberDetails(Syntax.EqualsOp, Syntax.ConstVal(val)) { Value = value };
            }
            else
            {
                return new MemberDetails(Syntax.EqualsOp, Syntax.ConstVal(value.ToString())) { Value = value };
            }
        }
